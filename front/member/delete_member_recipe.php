...
try {
    ...
    // 1. 檢查食譜是否存在且屬於當前用戶，避免使用者刪除不屬於自己的食譜
    ...
    if (!$result_check || $result_check->num_rows === 0) {
        ...
        echo json_encode(['status' => 'fail', 'message' => '食譜不存在或不屬於此使用者']);
        exit;
    }
    ...
    if ($affected_rows > 0) {
        http_response_code(200);
        echo json_encode(['status' => 'success', 'message' => '食譜已成功刪除！']);
    } else {
        http_response_code(404);
        echo json_encode(['status' => 'fail', 'message' => '食譜不存在或已被刪除']);
    }
