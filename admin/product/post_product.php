...
try {
  write_log("stmt: sql execute");
  // 回傳結果
  if ($stmt->execute()) {
    write_log("sql execute: ok");
    ...
    $data_return = [
      ...
      'image' => [
        'preview_image' => $preview_image_path ? [$preview_image_path] : [],
        'product_image' => is_array($product_image_paths) ? $product_image_paths : [],
        'content_image' => is_array($content_image_paths) ? $content_image_paths : [],
      ]
    ];
    write_log("send_json: data");
